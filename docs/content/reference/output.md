...
describes what users get. Pick a format with `-o`, or let twitch choose:
...
```bash
twitch <command> --fields id,title,url
```
...
```bash
twitch <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
twitch <command>            # a table, because this is a terminal
twitch <command> | wc -l    # JSONL, because this is a pipe
```
